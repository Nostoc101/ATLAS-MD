...
function siddLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [SIDD FREE BOT] ${new Date().toISOString()}: ${message}`);
}
                ...
                if (!connectedMessageSent) {
                    ...
                    try {
                        await conn.sendMessage(userJid, {
                            ...
                            caption: `> *╭────────────────◇*\n> *│✦ SIDD FREE BOT — ᴄᴏɴɴᴇᴄᴛᴇᴅ 🔥*\n> *│✦ ᴛʏᴘᴇ ${prefix}menu ᴛᴏ sᴇᴇ ᴀʟʟ ᴄᴍᴅs 💫*\n> *│✦ ᴘʀᴇғɪx 『 ${prefix} 』*\n> *│ᴍᴏᴅᴇ〔${mode}〕*\n> *╰────────────────○*\n> *© ᴍade ɪn ʙy sidd ᴛechx*`,
                            contextInfo: {
                                mentionedJid: [],
                                forwardingScore: 1,
                                isForwarded: true,
                                forwardedNewsletterMessageInfo: {
                                    ...
                                    newsletterName: config.BOT_NAME || 'SIDD FREE BOT',
                                    serverMessageId: 143
                                }
                            }
                        }, { quoted: fakevCard });
              ...
              try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© ᴄreate ʙy sidd ᴘrime',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:SIDD FREE BOT\nORG:SIDD FREE BOT;\nTEL;type=CELL;type=VOICE;waid=33751103165:33751103165\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© ᴄreate ʙy sidd ᴘrime' } }
                    }},
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'SIDD FREE BOT is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 SIDD FREE BOT — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
